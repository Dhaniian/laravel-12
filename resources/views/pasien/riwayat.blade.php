...
@section('nav-content')
    <ul class="nav">
        ...
        <li class="nav-item"><a href="{{ route('pasien.periksa') }}" class="nav-link"> <i class="nav-icon fas fa-th"></i>
                Periksa</a></li>
        <li class="nav-item"><a href="{{ route('pasien.riwayat') }}" class="nav-link"><i class="nav-icon fas fa-book"></i>
                Riwayat Periksa</a></li>
    </ul>
@endsection

@section('content')
    <h1>RIWAYAT PERIKSA {{ Auth::user()->name }} </h1>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @foreach ($periksa->obat as $obat)
                                                        ...
                                                        {{ number_format($obat->harga, 0, ',', '.') }}<br>
                                                    @endforeach
                                                    <hr>
                                                    <strong>Total Obat: Rp.
                                                        {{ number_format($totalObat, 0, ',', '.') }}</strong>
                                                </td>
                                            </tr>
                                            <tr class="bg-light">
                                                <th>Total Biaya</th>
